...
function CalculaIMC(_altura, _peso) 
{
    ...
    // return and convert a number into string, keeping only two decimal.
...
btcalcular.addEventListener("click", function(){
    ...
    let valor = CalculaIMC(txtaltura.value, txtpeso.value);
    let msg_valor = ranking(CalculaIMC(txtaltura.value, txtpeso.value));
...
txtaltura.addEventListener("keypress", function(event){
